...
import {
  ExcelUploadError,
  ExcelUploadResponse,
} from "../types/excelUploadResponse";
...
import {
  AdminQueryKeys,
  TABLE_LOAD_PREFIX,
} from "../consts/adminTableQueryKeys";
...
const ExcelPopUp: React.FC<ExcelPopUpProps> = ({
  ...
  const { mutate } = useMutation<ExcelUploadResponse, ExcelUploadError, any>(
    ...
    async (file: { excel: string }) =>
      (
        await filesUploader.post<ExcelUploadResponse>(uploadUrl, {
          file: file.excel,
        })
      ).data,
    {
    ...
    if (isExcelUploadError(error)) {
      ...
      displayErrorPopup(
        error.type === "file-size"
          ? [i18n.errors.fileSize]
          : [excelTexts.errorFileType]
      );
    }
  ...
  function formatErrors(errorData: ExcelUploadError) {
    return errorData?.data.errors
      ? formatExcelErrors(errorData.data.errors)
      : null;
  }
  // type guard for error
  function isExcelUploadError(
    error: ExcelUploadError | UploadError
  ): error is ExcelUploadError {
  ...
  async function handleExcelFileExampleDownload() {
    const { data } = await axios(`/api/excel-file-example/${exampleFileUrl}`, {
        ...
        <div className="form-Submit-continuer">
          {exampleFileUrl && (
            <Button
              ...
              onClick={handleExcelFileExampleDownload}
            >
